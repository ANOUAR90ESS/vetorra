...
const ClientLayout: React.FC<ClientLayoutProps> = ({
  ...
  // Logic to show ads: Only if no user or user has 'free' plan
  ...
  useEffect(() => {
    ...
    switch(currentView) {
        ...
        case AppView.PRICING:
            title = "Pricing & Plans - VETORRE";
            break;
        case AppView.PAYMENT:
            title = "Checkout - VETORRE";
            break;
  ...
  const handleNavigation = (view: AppView, pageId?: string) => {
      ...
      const viewToPath: Record<AppView, string> = {
        [AppView.HOME]: '/',
        [AppView.FREE_TOOLS]: '/free-tools',
        [AppView.PAID_TOOLS]: '/paid-tools',
        [AppView.TOP_TOOLS]: '/top-tools',
        [AppView.LATEST_NEWS]: '/news',
        [AppView.PRICING]: '/pricing',
        [AppView.PAYMENT]: '/payment',
        [AppView.SEARCH_RESULTS]: '/search',
        [AppView.PAGES]: pageId ? `/page/${pageId}` : '/pages',
        [AppView.ADMIN]: '/admin',
        [AppView.LIVE_CHAT]: '/live-chat',
        [AppView.VEO_STUDIO]: '/veo-studio',
        [AppView.IMAGE_STUDIO]: '/image-studio',
        [AppView.SMART_CHAT]: '/smart-chat',
        [AppView.AUDIO_LAB]: '/audio-lab',
      };
  ...
  const getFilteredToolsForView = (view: AppView, baseTools: Tool[]) => {
    ...
    // 2. Apply Page-Specific Logic
    switch (view) {
        case AppView.FREE_TOOLS:
            return filtered.filter(t => {
                const p = (t.price || '').toLowerCase();
                return p.includes('free') || p.includes('0') || p.includes('trial') || p.includes('open source');
            });
        case AppView.PAID_TOOLS:
            return filtered.filter(t => {
                const p = (t.price || '').toLowerCase();
                // If it doesn't mention free/trial, assume paid. Or if it has $ symbol.
                const isFree = p.includes('free') || p.includes('0') || p.includes('trial');
                const hasPrice = p.includes('$') || p.match(/\d/);
      ...
      <div className="flex-1 lg:ml-64 flex flex-col h-screen overflow-hidden relative">
        ...
        <main className="flex-1 overflow-y-auto bg-zinc-950 scroll-smooth">
          <div className="p-4 lg:p-8 min-h-full flex flex-col">
            ...
            {/* Top Advertisement - Only show for free plan */}
            {showAds && (
                    ...
                    {categories.map(cat => (
                      <button
                        ...
                        className={`px-4 py-1.5 rounded-full text-sm font-medium whitespace-nowrap transition-all border ${
                          categoryFilter === cat 
                            ? 'bg-zinc-900 border-indigo-500/20 shadow-[0_0_15px_rgba(99,102,241,0.15)]' 
                            : 'bg-zinc-900 border-transparent text-zinc-400 hover:bg-zinc-800'
                        }`}
                      >
                        {categoryFilter === cat ? (
                            <span className="bg-clip-text text-transparent bg-gradient-to-r from-indigo-400 via-purple-400 to-purple-500 font-bold">
                                {cat}
                            </span>
                        ) : (
                            cat
            ...
            {/* Bottom Advertisement - Only for free plan */}
            {showAds && (
