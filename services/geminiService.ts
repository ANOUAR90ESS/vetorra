...
// Type definitions (to avoid importing @google/genai on client)
const Type = {
  STRING: "string",
  ARRAY: "array",
  OBJECT: "object"
...
// Helper to call our serverless API
...
export const generateAIImage = async (prompt: string, aspectRatio: string = "16:9"): Promise<string> => {
    try {
        const result = await callGeminiAPI("generateImage", {
            prompt,
            aspectRatio,
            imageSize: "1K"
        });
...
export const generateDirectoryTools = async (count: number = 3, category?: string): Promise<Tool[]> => {
  ...
  const result = await callGeminiAPI("generateTools", {
    prompt,
    schema: {
      type: Type.ARRAY,
      items: {
  ...
  const toolsData = JSON.parse(result.text || "[]");
...
export const generateNewsDetails = async (topic: string): Promise<Partial<NewsArticle>> => {
    ...
    const result = await callGeminiAPI("generateNews", {
        prompt,
        useSearch: true,
        schema: {
            type: Type.OBJECT,
...
export const generateDirectoryNews = async (count: number = 3): Promise<NewsArticle[]> => {
    ...
    const result = await callGeminiAPI("generateDirectoryNews", {
        prompt,
        schema: {
            type: Type.ARRAY,
            items: {
    ...
    const newsData = JSON.parse(result.text || "[]");
...
export const intelligentSearch = async (query: string, tools: Tool[], news: NewsArticle[]): Promise<{ toolIds: string[], newsIds: string[] }> => {
    ...
    try {
        const result = await callGeminiAPI("intelligentSearch", {
            prompt,
            schema: {
                type: Type.OBJECT,
                properties: {
        ...
        return JSON.parse(result.text || '{"toolIds": [], "newsIds": []}');
    } catch (e) {
...
export const generateMultiSpeakerSpeech = async (script: string, speaker1Config: {name: string, voice: string}, speaker2Config: {name: string, voice: string}) => {
    // For now, use simple TTS (multi-speaker needs special handling in API)
    const result = await callGeminiAPI("generateSpeech", {
        text: `TTS the following conversation:\n${script}`,
        voice: speaker1Config.voice
    });
    return result.response;
};
...
export const generateToolTutorial = async (tool: Tool): Promise<TutorialSection[]> => {
    ...
    const modules = JSON.parse(result.text || "[]");
...
export const analyzeToolTrends = async (tools: Tool[]): Promise<string> => {
  ...
  return result.response?.text || "Unable to generate analysis.";
}

// --- Video Generation (kept as placeholder - needs special handling) ---
export const generateVideo = async (prompt: string, imageBase64?: string, aspectRatio: '16:9' | '9:16' = '16:9') => {
  console.warn("Video generation not yet implemented via API");
  return null;
};

export const pollVideoOperation = async (operation: any) => {
  console.warn("Video polling not yet implemented via API");
  return null;
};
...
export const editImage = async (prompt: string, imageBase64: string) => {
  console.warn("Image editing not yet implemented via API");
  return null;
};
